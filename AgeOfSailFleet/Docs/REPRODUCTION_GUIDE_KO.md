...
- 단일·드래그·추가·전체 선택, 우클릭 대형 이동과 공격 명령. 선택 함선
  아래에는 회전하는 밝은 점선 수면 링, 드래그 중에는 비어 있는 사각
  외곽선, 이동 지점에는 짧게 수렴하는 3D 화살표가 표시된다.
- 풍향 기반 속도, 측면 포격 위치 선정, 충돌 회피, 포탄 탄도, 피해,
  파괴 효과와 침몰
- 절차적 바다, 이중 파도 노멀, 항적, 총구 화염, 절제된 화약 연기 선체
  피격, 수중 피탄, 크게 보이는 포탄과 탄도 트레일, 침몰 중 수선 거품
- Blender로 만든 3D 표현과 진영별 8방향 2D 스프라이트 모드
- 동일 타원 구멍을 공유하는 함장 초상화, Bronze/Silver/Gold 로킷 프레임,
  함장명·함선명, 이미지 전용 날짜 활자, 풍향 화살표 HUD
- `V` 키 전략 카메라·자유 비행 토글
- 별도 HUD 없는 런타임 배속: `Z`는 1배 복귀, `X`는 0.5배 감소,
  `C`는 1.5배 증가하며 안전 범위는 0.5–8배다.
...
4. `AShipWakeActor`, `ASinkingFoamActor`, `AFlipbookEffectActor`
5. `AFleetMoveCommandMarker`, `AFleetCameraPawn`, `AFleetPlayerController`
...
| `AFleetPlayerController` | 선택, 대형 이동, 공격 명령, 카메라 모드, 시간 배속 |
| `AFleetCameraPawn` | 함대 전략 카메라와 복원 가능한 자유 비행 |
| `AFleetMoveCommandMarker` | 이동 지점으로 수렴하는 굵은 3D 화살표 8개 |
| `ASailOceanActor` | 270,000uu 절차 파도 Grid와 파봉 색 |
| `AShipWakeActor` | 속도 기반 2열 거품 Ribbon |
| `ASinkingFoamActor` | 침몰 선체와 수면의 교차선을 따르는 애니메이션 거품 |
| `ACannonballActor` | 탄도, 가시적 포탄·Trail, 선체·수면 피격 |
| `AFlipbookEffectActor` | 총구·수면 플립북과 다층 절차형 선체 피격 연기 |
| HUD·Title Widget | 바인딩과 표현만 담당 |

타이틀 전환 시간은 타이틀 페이드 0.45초, 암전 0.55초, 완전 암전 상태에서
전투 시작, 다시 밝아지는 시간 0.70초다. 타이틀 페이드 중 두 둥근
체크박스의 렌더 알파도 직접 갱신하고 0에서 `Collapsed` 처리해야 캐시된
원형 외곽선이 암전 단계에 남지 않는다.
...
- 바다 Geometry는 181×181, Cell 1500uu, 전체 폭 270,000uu다. 각 변을
  3배로 늘려 면적은 기존의 9배지만 초기 함선 배치는 바꾸지 않는다.
...
- 총구·수면 이펙트는 함대 카메라에서도 보여야 한다. 호출부의 효과
  배율을 두 번 `/100` 하지 않는다.
- 선체 피격은 원본 플립북의 초기 부분만 0.17초 미만, 48% 크기의 섬광으로
  사용한다. 이후 불규칙한 절차 메시 연기 4겹이 약 1.65–2.23초 동안 서로
  다른 시점에 팽창·상승·회전·페이드한다. `M_ShipWake`와 채도가 낮은
  회갈색 Vertex Color를 사용하고 만화풍 폭발 시트 전체는 재생하지 않는다.
- 침몰 거품은 액터 하나에 방사형 절차 메시 패치 16개를 고정 풀로 둔다.
  1·2·3급 수선 반치수는 각각 `1500×500×350`, `1280×430×315`,
  `1080×350×275`다. 수면 평면을 회전한 타원체의 로컬 공간으로 변환해
  교차 원을 샘플하고 패치는 수면에 수평으로 유지한다. 패치마다 위상,
  회전, 비균일 스케일, 팽창, 페이드를 다르게 한다. 선체와 수면의 교차가
  사라지면 마지막 거품을 1.4초 동안 페이드한 뒤 액터를 제거한다.
...
배속은 0.5–8배로 제한하고 별도 HUD 표시는 만들지 않는다.
...
9. `7424d38` — 절차 바다 면적을 기존의 9배로 확장
10. `8e663a5` — 비행기식 반전이 아닌 자연스러운 자유 카메라 상하 시점
11. `2bb6600` — 수렴형 3D 이동 명령 표식
12. `63a6c77` — 회전 선택 링과 박스 선택 외곽선
13. `70d3125` — 포탄·트레일·총구 화염·피격 가시성 강화
14. `5462f83` — 침몰 거품, 화약 연기, 굵어진 이동 화살표
15. `8c04b12` — 라디오 버튼을 타이틀과 함께 페이드·숨김

대형 통합 커밋 내부의 실제 작업 순서는 Git만으로 복원할 수 없다. 6~7절은
최종 소스 의존성으로 추론한 가장 안전한 재현 순서다.
위 시간 배속 바인딩은 현재 소스 상태에 포함돼 있다. 이 문서 갱신을
커밋할 때 해당 최종 커밋 해시를 이 목록에 기록한다.
...
- 빗나가면 수면 피격, 명중하면 선체 피격이 보이고 포탄·Trail도 넓은
  카메라에서 읽힌다.
- 선체 피격은 짧은 섬광 뒤 절제된 다층 화약 연기가 나타나며 만화풍
  폭발 전체를 반복하지 않는다.
- 침몰 중인 함선은 변화하는 수면 접촉 윤곽에 여러 회전·확대 거품이
  생기며 완전히 잠긴 뒤 짧게 페이드한다.
- 선택 함선의 밝은 점선 링이 회전하고, 드래그 사각형 내부는 비어 있으며,
  이동 수렴 화살표는 넓은 카메라에서도 읽힌다.
- 날짜 간격이 좁고 어두운 배경이 없으며 풍향 화살표만 보인다.
- 타이틀의 두 라디오 원형도 함께 페이드하며 암전 중 외곽선이 남지 않는다.
- 자유 비행 종료 시 `V`를 누르기 전 전략 카메라 상태가 정확히 복원된다.
- `Z`, `X`, `C`가 각각 1배 복귀, 0.5배 감소, 1.5배 증가로 동작한다.
...
- **타이틀 페이드 뒤 라디오 원형이 남음:** 체크박스의 Render Opacity를
  타이틀 알파와 함께 직접 갱신하고 알파가 0이면 두 컨트롤을
  `Collapsed` 처리한다. 상위 Border 알파만 바꾸면 Rounded Box의 캐시된
  Draw Element가 한 프레임 남을 수 있다.
